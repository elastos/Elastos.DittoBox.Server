#! /bin/sh
### BEGIN INIT INFO
# Provides:          elaoc-agentd
# Required-Start:    $remote_fs $syslog
# Required-Stop:     $remote_fs $syslog
# Default-Start:     2 3 4 5
# Default-Stop:      0 1 6
# Short-Description: Starts the Elastos Owncloud Agent daemon
# Description:       Starts the Elastos Owncloud Agent daemon
### END INIT INFO

# PATH should only include /usr/* if it runs after the mountnfs.sh script
PATH=/sbin:/usr/sbin:/bin:/usr/bin
DESC="Elastos Owncloud Agent daemon"
NAME=elaoc-agentd
DAEMON=/usr/bin/$NAME
CONFIG_FILE=/etc/elaoc/elaoc-agent.conf
